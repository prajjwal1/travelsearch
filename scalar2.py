...
def Create_Scalar_Clustering(results, Query_String ):
    ...
    for doc_no,doc in enumerate(results["crawl"]):
    #     Documents_List.append(doc['content'].replace("\n", " "))
        Documents_terms.extend(doc['text'].replace("\n", " ").split(" "))
        doc_dict[doc_no] = doc['text'].replace("\n", " ").split(" ")
...
def scalar_main():
    ...
    with open("testir2.json", "r") as read_file:
      data = json.load(read_file)
    with open('/people/cs/s/sanda/cs6322/resourcesIR/stopwords', 'r') as filehandle:
    ...
    query = "beds bath rooms two"
    Expanded_Query  = Create_Scalar_Clustering(data, query)
